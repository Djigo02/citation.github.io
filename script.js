...
let citations = [
  ...
  ["Chacun ses responsabilites soit il les assume soit il les fuit","M. Djigo"]
];

const citation = document.querySelector("#citation");
const auteur = document.querySelector("#auteur");
const nouveau = document.querySelector("#nouveau");
var random = "";//la ligne de la citation dans le tableau
window.addEventListener("load",()=>{
    //Au chargement genere automatiquement une citation
...
nouveau.addEventListener('click',()=>{
  //Au click s'assure que la citation actuel est differente de celle qui sera generer.
  let rand;
    do {
      //Regenere une autre citation si la citation generer et l'actuelle sont identiques.
      rand = Math.floor(Math.random()*citations.length);
      console.log(rand,random);
      if(rand!= random){
        random = rand;
        citation.textContent = citations[rand][0];
        auteur.textContent = citations[rand][1];
        break;
      }
    } while (rand == random);
});